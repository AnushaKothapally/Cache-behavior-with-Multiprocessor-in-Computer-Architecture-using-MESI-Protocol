 ...
 * @author Lakshman
 */
public class processr1L1controler {

    Queues ques;

    public processr1L1controler(Queues que) {
        this.ques = que;
    }
    int i = 0;
    String[] c;
    ...
    public void readfromP1reqq() {
        //L1cache[42][0]="10001";
        //vb[42][0]=true;
        // System.out.println("in L1Cache from P1 request");
        Instr = ques.getP1reqtoP1c();
        int n = Instr.size();
        String MM=Instr.get(0);
        if(MM.contains("Load"))
        System.out.println("in L1Cache of P1 " + MM.substring(0,24));
        else
            System.out.println("In L1Cache of P1  " + MM);
        s = Instr.get(0);//get the ith item of the list anf store it in a string
        c = s.split(" ");
        k = B2Iconvert(c);
        ques.getP1reqtoP1c().remove(0);
       // System.out.println("**removed is:" + ques.getQueue().size());

    }
//        public void readfromP1resq() {
//
//            Instr = ques.getP1ResQueue();
//            int n = Instr.size();
//            System.out.println("in L1Cache from P1 response"+Instr.get(0));
//          //  s = Instr.get(0);//get the ith item of the list anf store it in a string
//            s = Instr.get(0);
//            c = s.split(" "); 
//            k = B2Iconvert(c);
//            ques.getP1ResQueue().remove(0);
//
//        }
    ...
    public String B2Iconvert(String[] d) {
        ...
        if (d[0].equalsIgnoreCase("R1")) {
            ...
            if (k == null) {
                System.out.println("No data found in P1 L1 Cache also");
                s = d[0] + " " + d[1] + " " + d[2] + " " + "I";
                ques.setP1RestoBusQueue(s);
            }
        } else if (d[0].contains("Load") || d[0].contains("S1")) {
            //String data=
            loading(a, b, c, tag, index, offset, d);
        } else if (d[0].contains("W1")) {
            ...
            if (isdatafound) {
                //String input = "W1" + " " + d[1] + " " + d[2];
                ques.setP1CtoP1DQueue(s);
            } else {
                String input = "BusW1Ex" + " " + d[1] + " " + d[2];
                ques.setP1ReqtoBusQueue(input);
            }
...
//        }
    ...
    public String readForAnotherprocessor(String a, String b, String c, int tag, int index, int offset, String[] add) {
        ...
        if ((a.equals(L1cache[index][0])) && (vb[index][0] == true)) {
            System.out.println("tag matched so checking in P1L1d");
            if (db[index][0] == true) {
                String instru = add[0] + " " + add[1] + " " + add[2] + " " + "DB";
                db[index][0] = false;
                ques.setP1CtoP1DQueue(instru);
                System.out.println("Dirty bit is true: " + instru);
            } else {
                ques.setP1CtoP1DQueue(s);
            }
            //System.out.println("tag matched so checking in P1L1d" + L1cache);
            //ques.setP1CtoP1DQueue(s);
            ...
            //System.out.println("No data found in P1 L1 cache");
            return null;
        }
    ...
    public void loading(String a, String b, String c, int tag, int index, int offset, String[] d) {
       // System.out.println("Data loading into cache of P1 " + b);
        String input = null;
    ...
    public void invalidate(String a, String b, String c, int tag, int index, int offset, String[] add) {
        if ((a.equals(L1cache[index][0])) && (vb[index][0] == true) && db[index][0] == false) {
            System.out.println("Data present in P1L1d, invalidating:");
    ...
    public void notempty() {
        String received = ques.getP1DtoP1CQueue().get(0);
       // System.out.println("P1D2P1C called" + received);
        if (received.contains("BUS")) {
            ques.setP1RestoBusQueue(received);
        } else {
            ques.setOutput(received);
        }
        ques.getP1DtoP1CQueue().remove(0);
    }
}
